...
### Размер кирпича вдоль траектории
```js
...
### Учёт визуального зазора (`brickMargin = 0.97`)
```js
halfExtent = brickAlongPath * brickMargin / 2  // половина длины кирпича с учётом масштаба
step = brickAlongPath * brickMargin + gapM     // шаг между центрами кирпичей
```

`brickMargin` создаёт визуальный зазор ~3% между кирпичами (меш масштабируется на 0.97).
