...
Алгоритм размещает заданное количество кирпичей по периметру прямоугольного фундамента дома. Сначала принимаются все размеры, затем выполняется расстановка по правилам ниже.

---

## Входы алгоритма

1. **Размер кирпича** — ширина вдоль ряда `brickW` (x), длина (толщина стены) `brickL`, высота `brickH`.
2. **Количество кирпичей** — `distributionBrickCount` (N).
3. **Размер зазора** — невидимый «кирпич» E в мм (`brickGap`), конвертируется в метры `gapM` (обычно 1–3 мм).
4. **Размер дома** — половины сторон `L`, `W` (длина стены = 2L или 2W).

После принятия всех размеров алгоритм начинает расставлять кирпичи.

---

## Правила расстановки (три шага)

1. **Первый кирпич — замыкающий Z1 в углу дома.**  
   Ставится в первый угол. Длина Z1 от **x до 2x** (x = длина обычного кирпича вдоль ряда, т.е. `brickW`).  
   В коде: `lenAlong = min(2x, max(x, remaining))`, с ограничением `lenAlong ≤ remaining`.

2. **Следующий кирпич — обычный N1, затем N2, …**  
   После каждого кирпича (Z1 или N) добавляется зазор E, затем ставится следующий кирпич.

3. **Условие замыкающего Z.**  
   Если от края текущего кирпича до края дома (до следующего угла) остаётся **2x или меньше** (`remaining ≤ 2x`), ставится замыкающий кирпич Z длиной в этот остаток. Затем зазор E и продолжение со следующей стены/угла.

По периметру используется одна и та же длина вдоль ряда — **x = brickW** (не `max(brickW, brickL)`). Ряды чередуются со смещением на полкирпича; зазоры E задают расстояние между центрами соседних кирпичей.
...
// ВАЖНО: вдоль траектории длина меша = реальная длина кирпича (brickAlongPath),
// brickMargin применяется только поперёк (толщина, высота), а не по длине.
brickExtentM = brickAlongPath                 // фактическая длина кирпича N вдоль траектории
```
